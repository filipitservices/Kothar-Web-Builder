...
**Unsaved changes:** Editable routes (`/gallery/request/:id`, `/orders/:id/edit`, both ID-scoped builders, `/report-issue`) register with a central Pinia store (`stores/unsavedChanges.ts`). A client-only router guard cancels navigation when dirty and opens **`UnsavedChangesDialog`** (design-system modal in `app.vue`). The same store drives `beforeunload` for tab close/refresh. Successful submits call `requestAllowNext()` before `navigateTo` so the guard does not block intentional exits. **Sign-out** calls `prepareForAuthTerminatedNavigation()` in `useAuth` (closes any open unsaved dialog); the guard allows navigation when `auth` store reports no user and clears any stale `allowNext`, so post-logout `navigateTo` cannot trap the user. **`auth` middleware** clears the auth store when `/api/auth/me` returns no user before redirecting to login so stale “logged in” state does not block that redirect. **`/sites/:id`** is intentionally excluded until live-site persistence semantics are unified (see `composables/useUnsavedChanges.ts`).
