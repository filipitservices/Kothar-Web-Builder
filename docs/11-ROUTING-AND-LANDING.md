...
**Unsaved changes:** Editable routes (`/gallery/request/:id`, `/orders/:id/edit`, `/gallery/request/:id/builder`, `/orders/:id/builder`) register with a central Pinia store (`stores/unsavedChanges.ts`) using two explicit signals: `hasDirtyEdits` (diff-based form/layout/sketch changes) and `hasUnsavedSession` (active draft lifecycle). A client-only router guard (`plugins/unsaved-changes-guard.client.ts`) opens the global dialog when there is something to lose and the navigation is either **out of the editing flow** or **between paired request/order pages and the builder** (e.g. `/gallery/request/:id` ↔ `/gallery/request/:id/builder`, `/orders/:id/edit` ↔ `/orders/:id/builder`) — so a clean hop back to the request form does not prompt, but a **dirty** internal move does. The dialog offers **Stay**, optional **Stash** (session-only layout snapshot via `utils/builderSessionStash.ts` on builder routes), and **Discard**. The same store drives `beforeunload` for tab close/refresh. Successful submits call `requestAllowNext()` before `navigateTo` so the guard does not block intentional exits. **Sign-out** calls `prepareForAuthTerminatedNavigation()` in `useAuth` (closes any open unsaved dialog); the guard allows navigation when `auth` store reports no user and clears any stale `allowNext`, so post-logout `navigateTo` cannot trap the user. **`auth` middleware** clears the auth store when `/api/auth/me` returns no user before redirecting to login so stale “logged in” state does not block that redirect. **`/sites/:id`** is intentionally excluded until live-site persistence semantics are unified (see `composables/useUnsavedChanges.ts`).
