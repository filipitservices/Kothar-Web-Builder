...
**Unsaved changes:** Editable routes (`/gallery/request/:id`, `/orders/:id/edit`, `/gallery/request/:id/builder`, `/orders/:id/builder`) register with a central Pinia store (`stores/unsavedChanges.ts`) using two explicit signals: `hasDirtyEdits` (diff-based form/layout changes) and `hasUnsavedSession` (active draft lifecycle). A client-only guard prompts only when the user is **leaving the editing flow**; transitions within the same request/order scope (`edit ↔ builder` with the same `:id`) do not prompt. This ensures untouched drafts still warn on external exit, while internal flow moves remain uninterrupted. The same store drives `beforeunload` for tab close/refresh. Successful submits call `requestAllowNext()` before `navigateTo` so the guard does not block intentional exits. **Sign-out** calls `prepareForAuthTerminatedNavigation()` in `useAuth` (closes any open unsaved dialog); the guard allows navigation when `auth` store reports no user and clears any stale `allowNext`, so post-logout `navigateTo` cannot trap the user. **`auth` middleware** clears the auth store when `/api/auth/me` returns no user before redirecting to login so stale “logged in” state does not block that redirect. **`/sites/:id`** is intentionally excluded until live-site persistence semantics are unified (see `composables/useUnsavedChanges.ts`).
